...
this.createjs = this.createjs || {}, function () {
	"use strict";
	function a(a) {
		this.AbstractLoader_constructor(a, !1, createjs.AbstractLoader.SOUND), this.flashId = null
	}

	var b = createjs.extend(a, createjs.AbstractLoader), c = a;
	c._flash = null, c._preloadInstances = [], c.setFlash = function (a) {
		c._flash = a;
		for (var b = c._preloadInstances.length; b--;) {
			var d = c._preloadInstances.pop();
			d.load()
		}
	}, b.load = function () {
		if (null == c._flash)return void c._preloadInstances.push(this);
		this.flashId = c._flash.preload(this._item.src);
		var a = new createjs.Event(createjs.FlashAudioPlugin._REG_FLASHID);
		this.dispatchEvent(a)
	}, b.handleProgress = function (a, b) {
		this._sendProgress(a / b)
	}, b.handleComplete = function () {
		this._result = this._item.src, this._sendComplete()
	}, b.handleError = function (a) {
		this._handleError(a)
	}, b.destroy = function () {
		var a = new createjs.Event(createjs.FlashAudioPlugin._UNREG_FLASHID);
		this.dispatchEvent(a), this.AbstractLoader_destroy()
	}, b.toString = function () {
		return "[FlashAudioLoader]"
	}, createjs.FlashAudioLoader = createjs.promote(a, "AbstractLoader")
}(), this.createjs = this.createjs || {}, function () {
	"use strict";
	function a(a, b, d, e) {
		this.AbstractSoundInstance_constructor(a, b, d, e), this.flashId = null, null == c._flash && c._instances.push(this)
	}

	var b = createjs.extend(a, createjs.AbstractSoundInstance), c = a;
	c._flash = null, c._instances = [], c.setFlash = function (a) {
		c._flash = a;
		for (var b = c._instances.length; b--;) {
			var d = c._instances.pop();
			d._setDurationFromSource()
		}
	}, b.toString = function () {
		return "[FlashAudioSoundInstance]"
	}, b._removeLooping = function () {
		null != this.flashId && c._flash.setLoop(this.flashId, this._loop)
	}, b._addLooping = function () {
		null != this.flashId && c._flash.setLoop(this.flashId, this._loop)
	}, b._updateVolume = function () {
		null != this.flashId && c._flash.setVolume(this.flashId, this._volume)
	}, b._updatePan = function () {
		null != this.flashId && c._flash.setPan(this.flashId, this._pan)
	}, b._setDurationFromSource = function () {
		this._duration = c._flash.getDurationBySrc(this.src)
	}, b._interrupt = function () {
		null != this.flashId && (c._flash.interrupt(this.flashId), this.AbstractSoundInstance__interrupt())
	}, b._handleCleanUp = function () {
		c._flash.stopSound(this.flashId), this._sendEvent(createjs.FlashAudioPlugin._UNREG_FLASHID), this.flashId = null
	}, b._beginPlaying = function (a, b, d, e) {
		return null == c._flash ? !1 : (this.setPosition(a), this.setLoop(b), this.setVolume(d), this.setPan(e), this._paused = !1, this.flashId = c._flash.playSound(this.src, this._position, this._loop, this._volume, this._pan, this._startTime, this._duration), null == this.flashId ? (this._playFailed(), !1) : (this._muted && this.setMute(!0), this._sendEvent(createjs.FlashAudioPlugin._REG_FLASHID), this.playState = createjs.Sound.PLAY_SUCCEEDED, this._sendEvent("succeeded"), !0))
	}, b._pause = function () {
		null != this.flashId && (this._position = this._calculateCurrentPosition(), c._flash.pauseSound(this.flashId))
	}, b._resume = function () {
		null != this.flashId && c._flash.resumeSound(this.flashId)
	}, b._handleStop = function () {
		null != this.flashId && c._flash.stopSound(this.flashId)
	}, b._updateVolume = function () {
		var a = this._muted ? 0 : this._volume;
		c._flash.setVolume(this.flashId, a)
	}, b._calculateCurrentPosition = function () {
		return c._flash.getPosition(this.flashId)
	}, b._updatePosition = function () {
		null != this.flashId && c._flash.setPosition(this.flashId, this._position)
	}, b.handleSoundFinished = function () {
		this._loop = 0, this._handleSoundComplete()
	}, b.handleSoundLoop = function () {
		this._loop--, this._sendEvent("loop")
	}, createjs.FlashAudioSoundInstance = createjs.promote(a, "AbstractSoundInstance")
}(), this.createjs = this.createjs || {}, function () {
	"use strict";
	function a() {
		this.AbstractPlugin_constructor(), this.showOutput = !1, this._CONTAINER_ID = "flashAudioContainer", this._WRAPPER_ID = "SoundJSFlashContainer", this._container = null, this._flash = null, this.flashReady = !1, this._flashInstances = {}, this._flashPreloadInstances = {}, this._queuedInstances = [], this._capabilities = c._capabilities, this._loaderClass = createjs.FlashAudioLoader, this._soundInstanceClass = createjs.FlashAudioSoundInstance;
		...
		b.id = this._CONTAINER_ID, b.appendChild(document.createTextNode("SoundJS Flash Container")), a.appendChild(b);
		{
			var d = c.swfPath;
			swfobject.embedSWF(d + "FlashAudioPlugin.swf", this._CONTAINER_ID, "1", "1", "9.0.0", null, null, {AllowScriptAccess: "always"}, null, createjs.proxy(this._handleSWFReady, this))
		}
	}

	var b = createjs.extend(a, createjs.AbstractPlugin), c = a;
	c._REG_FLASHID = "registerflashid", c._UNREG_FLASHID = "unregisterflashid", c._capabilities = null, c.swfPath = "src/soundjs/flashaudio/", c.isSupported = function () {
		return createjs.BrowserDetect.isIOS || createjs.BrowserDetect.isAndroid || createjs.BrowserDetect.isBlackberry || createjs.BrowserDetect.isWindowsPhone ? !1 : (c._generateCapabilities(), null == swfobject ? !1 : swfobject.hasFlashPlayerVersion("9.0.0"))
		...
		this.flashReady || this._queuedInstances.push(a);
		var d = this.AbstractPlugin_register(a, b);
		return d.addEventListener(c._REG_FLASHID, createjs.proxy(this.registerPreloadInstance, this)), d.addEventListener(c._UNREG_FLASHID, createjs.proxy(this.unregisterPreloadInstance, this)), d
	}, b.removeSound = function (a) {
		var b = createjs.indexOf(this._queuedInstances, a);
		-1 != b && this._queuedInstances.splice(b, 1), this.AbstractPlugin_removeSound(a)
	}, b.removeAllSounds = function () {
		this._queuedInstances.length = 0, this._flashInstances = {}, this._flashPreloadInstances = {}, this.AbstractPlugin_removeAllSounds()
	}, b.create = function (a, b, d) {
		var e = this.AbstractPlugin_create(a, b, d);
		return e.on(c._REG_FLASHID, this.registerSoundInstance, this), e.on(c._UNREG_FLASHID, this.unregisterSoundInstance, this), e
	}, b.toString = function () {
		return "[FlashAudioPlugin]"
	}, b._handleSWFReady = function (a) {
		this._flash = a.ref
	}, b._handleFlashReady = function () {
		this.flashReady = !0, this._loaderClass.setFlash(this._flash), this._soundInstanceClass.setFlash(this._flash);
	...
	}, b.registerPreloadInstance = function (a) {
		this._flashPreloadInstances[a.target.flashId] = a.target
	}, b.unregisterPreloadInstance = function (a) {
		delete this._flashPreloadInstances[a.target.flashId]
	}, b.registerSoundInstance = function (a) {
		this._flashInstances[a.target.flashId] = a.target
	}, b.unregisterSoundInstance = function (a) {
		delete this._flashInstances[a.target.flashId]
	}, b.flashLog = function (a) {
		try {
			this.showOutput && console.log(a)
		} catch (b) {
		}
	}, b.handleSoundEvent = function (a, b) {
	...
	}, b.handlePreloadEvent = function (a, b) {
	...
	}, b.handleErrorEvent = function () {
	}, createjs.FlashAudioPlugin = createjs.promote(a, "AbstractPlugin"), createjs.FlashPlugin = createjs.FlashAudioPlugin
}(), this.createjs = this.createjs || {}, function () {
	var a = createjs.FlashAudioPlugin = createjs.FlashAudioPlugin || {};
	a.version = "NEXT", a.buildDate = "Thu, 11 Dec 2014 23:16:15 GMT"
}();